...
def main(args: argparse.Namespace):
    ...
    train_source_dataset = dataset(root=args.root, task=args.source, download=True, transforms=train_transforms)
    train_source_loader = DataLoader(train_source_dataset, batch_size=args.batch_size,
                                     shuffle=True, num_workers=args.workers, pin_memory=True, drop_last=True)
    val_target_dataset = dataset(root=args.root, task=args.target, download=True, transforms=val_transforms)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--train-size', nargs='+', type=int, default=(240, 240),
                        help='the input and output image size during training')
    parser.add_argument('--test-input-size', nargs='+', type=int, default=(240, 240),
                        help='the input image size during test')
    parser.add_argument('--test-output-size', nargs='+', type=int, default=(240, 240),
